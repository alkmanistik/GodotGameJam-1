...
# Локации
	0) Логово злодея 
	Мы появляемся здесь, тут игра начнётся и тут она и завершиться, так же сюда надо будет заходить,
	чтобы получить биллет на другую локацию.
	1) Город
...
### Для локации Город
	...
	2)Кот в маске - кот в маске другого животного, нужно будет поговорить с ним и скачать,
...
### Для локации Научная лаборатория
	...
	3)Кот гигачад - учёные срздавали ген самого сильного кота, и они смогли создать из пробирки такого,
	...
	у кота с валерьянкой), принися её он выйдет из лаборатории
### Для локации Долина/лес
	...
	подойди к удачке и порыбыч и он придёт к тебе
	3)Кот в коробке - стоят рядом три коробки, и котик выпрыгивает из одной из них 
	и обратно нужно успеть поймать его
	4)Кот фермер - ему нужно помочь с чючелом, оно не такое страшное, нужно принести 
...
# UI
	...
	Прогресс локации будет справо снизу, в нём будет количество котов собранных
	 вами из количество всех на этой локации
...
# Концовки
	...
	но решает запустить ретуал, вы думаете, что всё потеряно, но на самом деле он призывает этих кошек сюда и хочет за ними заботиться
